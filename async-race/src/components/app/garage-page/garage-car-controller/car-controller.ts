...
class CarController {
  ...
  timer(): void {
    ...
    const interval = setInterval(() => {
      document.querySelector('.race-timer').innerHTML = `Race Timer:${counter}`
      counter--
      if (counter === -1) {
  ...
  showAnnouncement(car: WinnerItem): void {
    ...
    document.querySelector('.announcement-name').innerHTML = `${carName} Won!`
    document.querySelector('.announcement-seconds').innerHTML = `${(this.winner as WinnerItem).time} s`
  }
}
