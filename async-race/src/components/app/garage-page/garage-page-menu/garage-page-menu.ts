...
class GarageMenu {
  ...
  generateCars(brands: Array<string>, models: Array<string>): void {
    for (let i = 1; i < 100; i++) {
      const car = {
        name: `${brands[Math.floor(Math.random() * 9)]} ${models[Math.floor(Math.random() * 9)]}`,
  ...
  async selectCar(id: number): Promise<void> {
    this.selectedCar = await this.service.requestCar(id)
    document.querySelector('.update-name')?.removeAttribute('disabled')
    document.querySelector('.update-btn')?.removeAttribute('disabled')
    ;(document.querySelector('.update-name') as HTMLInputElement).value = this.selectedCar?.name
    ;(document.querySelector('.update-color') as HTMLInputElement).value = this.selectedCar?.color
  }
